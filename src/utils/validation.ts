...
export class ConfigValidator {
  ...
  static validateStorageConfig(config: MainframeConfig): string[] {
    ...
    // Validate IPFS configuration if present
    if (storage.primary === 'ipfs' || storage.fallback?.includes('ipfs')) {
      if (!storage.ipfs?.gateway) {
        errors.push('storage.ipfs.gateway is required when using IPFS');
      } else if (!this.isValidUrl(storage.ipfs.gateway)) {
        errors.push('storage.ipfs.gateway: Invalid URL format');
      }
    }

    // Validate Arweave configuration if present  
    if (storage.primary === 'arweave' || storage.fallback?.includes('arweave')) {
      if (!storage.arweave?.gateway) {
        errors.push('storage.arweave.gateway is required when using Arweave');
